	...
	<div>
		<div style="display: flex;">
			<div class="demo-upload-list" v-for="(item,index) in value" :key="index">
				...
				<div v-else>
					<i-circle v-if="item.percent||item.percent==0" :size="size" :percent="item.percent" :stroke-color="item.percent==100?'#5cb85c':'#2db7f5'"
			...
			upcloud(filePath, index) {
				...
				uniCloud.uploadFile({
					...
					onUploadProgress: function(progressEvent) {
						that.value[index].percent = Math.round(
